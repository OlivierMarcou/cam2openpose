...
parser.add_argument('--cap', default=0, type=int, help='auto register')
...
while 1:
    ...
    for i in range(len(BODY_PARTS)):
        ...
        overlay = frame.copy() 
        cv.rectangle(overlay, pt1=(0,0), pt2=(frameWidth,frameHeight), color=(0,0,0), thickness=-1)
        alpha = 0.9  # Transparency factor.    
        frame2 = cv.addWeighted(overlay, alpha, frame, 1 - alpha, 0) 
    ...
    for pair in POSE_PAIRS:
        ...
        if points[idFrom] and points[idTo]:
            ...
            cv.ellipse(frame2, points[idFrom], (3, 3), 0, 0, 360, (0, 0, 255), cv.FILLED)
            cv.ellipse(frame2, points[idTo], (3, 3), 0, 0, 360, (0, 0, 255), cv.FILLED)

    t, _ = net.getPerfProfile()
    freq = cv.getTickFrequency() / 1000
    cv.putText(frame2, '%.2fms' % (t / freq), (10, 20), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0))

    cv.imshow('OpenPose using OpenCV', frame2)
